...
# Check for decrementing sequences
...
function PasswordMatcher {
    param([string]$Pin)
    $PassList = Get-Content -Path "Files\PassList.txt"
    ...
    foreach ($pass in $clean) {
        ...
        if ($Pin -match ("^{0}$" -f [regex]::Escape($pass))) {
            $matched = $true
        }
    }
    return $matched
...
#================================================ # 
# MARK: BitLocker PIN GUI Function
#================================================ #
function BitLockerGuiLauncher {
    $global:securePin = $null
    ...
    $formBitLockerStartupPIN_Load = {
        ...
        # Retrieve native BitLocker PIN policies
        try {
            $global:MinimumPIN = ""
    ...
    $buttonSetPIN_Click = {
        ...
        if ($textboxNewPin.Text -eq $textboxRetypedPin.Text){
          if ($(Test-IncrementCharacter -CharString $textboxNewPin.Text) -or $(Test-DecrementCharacter -CharString $textboxNewPin.Text)) {
            ...
            $plainPin = $textboxNewPin.Text

            # Convert immediately
            $global:securePin = ConvertTo-SecureString -String $plainPin -AsPlainText -Force

            # Zeroise UI + plaintext references
            $textboxNewPin.Clear()
            $textboxRetypedPin.Clear()
            $plainPin = $null
    ...
    # Enter key handler for Retype PIN textbox
    ...
    # Enter key handler for New PIN textbox
    ...
    # Deregister event handlers on form close to prevent memory leaks
...
##================================================
    ...
    $formBitLockerStartupPIN.add_FormClosing({
        if (-not $global:securePin) {
